...
import { Plus, Pencil, Trash2, X, Check, ShoppingBag, TrendingUp } from 'lucide-react'
...
const MEDIOS_PAGO = ['Cta. DNI', 'Mercado Pago', 'Naranja', 'Efectivo']
...
const MEDIO_ICONS: Record<string, string> = {
  'Cta. DNI':     '🏦',
  'Mercado Pago': '💙',
  'Naranja':      '🟠',
  'Efectivo':     '💵',
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-[11px] font-bold tracking-[1.5px] uppercase text-brand-muted mb-1.5">Medio de pago</label>
                ...
                {Object.entries(data!.por_categoria)
                  ...
                  .map(([cat, total]) => (
                    <div key={cat} className="flex items-center gap-2 rounded-xl px-3 py-1.5"
                      ...
                      <span className="text-xs font-bold text-brand-body">{fmt$(total)}</span>
                    </div>
                  ))}
              </div>
            </div>
                ...
                <table className="w-full">
                  ...
                  <tbody>
                    {ventas.map((v, i) => (
                      <tr key={v.id}
                        ...
                        <td className="px-3 py-3 text-center text-base" title={v.medio_pago}>
                  ...
                  <tfoot>
                    <tr style={{ background: `${SAGE}14` }}>
                      ...
                      <td colSpan={3} className="px-4 py-3 text-right text-xs text-brand-muted">
      ...
      {modal.open && (
        <VentaModal
          initial={modal.venta ? {
            ...
            medio_pago:      modal.venta.medio_pago,
